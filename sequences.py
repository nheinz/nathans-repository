...
<<<<<<< Updated upstream
base = random.randint(0,3) # For the base, I used the module random.randint because this module will generate a random integer for us depending on the range we give it.
# In this instance, I used the range (0,4). I will elaborate on this in a later comment.

exponent = random.randint(0,3) # Once again, I used the random.randint module to generate a single digit integer from 0 to 4. This range will be explained later.
=======
base = random.randint(0,3) # For the base, I used the module random.randint because this module will generate a random integer for us depending on the range we give it.
# In this instance, I used the range (0,4). I will elaborate on this in a later comment.
...
# For the conflict resolution competency, I'm changing the random.randint range from (0, 4) to (0, 3) for base and exponent
>>>>>>> Stashed changes
